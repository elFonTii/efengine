...
layout(binding = 0) uniform sampler2D uImage;      // imagen a desenfocar

layout(std140, binding = 4) uniform PassParams {
    vec4 uParams;   // x = 1 pasada horizontal, 0 vertical
};
...
void main() {
    ...
    vec2 dir = (uParams.x > 0.5) ? vec2(texel.x, 0.0) : vec2(0.0, texel.y);
